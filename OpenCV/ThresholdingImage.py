...
def Thresholding(imgfile):
    img = cv2.imread(imgfile, cv2.IMREAD_GRAYSCALE)

    #threshold parameter = (image, threshold value, to what value, method to use)
    #return value : ret is threshold value that used, thr is changed image
...
def AdaptedThresholding(imgfile):
    img = cv2.imread(imgfile, cv2.IMREAD_GRAYSCALE)

    ret, thr1 = cv2.threshold(img, 127, 255, cv2.THRESH_BINARY)
    #Mean_C method : Consider threshold value as mean of nearby pixels - C(correction factor)
    #Gaussian_C method : Use gaussian filter - C
    ...
    #Adapted Threshold method is more efficient. Minimize noise, Refined Image.
    for i in range(4):
...
def OtsuThresholding(imgfile):
    img = cv2.imread(imgfile, cv2.IMREAD_GRAYSCALE)

    ret, thr1 = cv2.threshold(img, 127, 255, cv2.THRESH_BINARY)
    #Otsu Binarization(cv2.threshold() + cv2.thresh_otsu flag value)
    ...
    titles = ['original noisy', 'Histogram', 'General Thresholding',
